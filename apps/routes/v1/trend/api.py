...
from apps.routes.v1.trend.utils import extract_trendy_data

ns = Namespace("api/v1/trend", "Get alerts based on their class and lightcurve trend")
...
ARGS = ns.model(
    "trend",
    {
        ...
        "trend": fields.String(
            description="Desired trend among: rising, fading. For `class=CTAOBlazar`, the trend `low_state` is also available.",
            example="rising",
            required=True,
        ),
        "n": fields.Integer(
...
class Trend(Resource):
    def get(self):
        """Retrieve alerts from ZTF based on their class and lightcurve trend"""
    ...
    def post(self):
        ...
        out = extract_trendy_data(payload)
